...
bool Ellipsoid::IntersectTest(const Ray& a_ray, IntersectResponse& a_ir) const
{
	...
	else
	{
		...
		{
			return false;
		}					//Both intersection points behind ray origin
//Now that we have closest positive intersection point
		...
		Vector3 sn = Normalize(hp.xyz());						//normalise this point provides direction of surface normal>>>changed in Creating a scene from: a_surfNormal = Normalize(hp.xyz());
		hp = m_Transform * hp;									//Multiply point by inverse transform matrix to get in world space>>>changed in Creating a scene from: hp = invTx * hp;
		a_ir.HitPos = Vector3(hp.x, hp.y, hp.z);				//Nearest hitpoint on surface of ellipsoid to ray
		a_ir.SurfaceNormal = sn.Normalized(); // Normalize(normalMatrix * sn);		//Convert Normal into world space normal by multipling with normal matrix>>>Changed (C_A_S) from: a_surfNormal = Normalize(normalMatrix * a_surfNormal).xyz();
